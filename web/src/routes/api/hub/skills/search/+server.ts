...
/**
 * GET /api/hub/skills/search
 * Search for skills across external hub sources.
 */
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    const sortParam =
      url.searchParams.get("sort")?.trim().toLowerCase() || "relevance";
    ...
    const invalidSources = rawRequestedSources.filter((v) => !isProviderKey(v));
    ...
    const enabledSources = PROVIDERS.filter(
      (p) => sourcesConfig[p]?.enabled !== false,
    );
    const selectedSources =
      requestedSources.length > 0 ? requestedSources : enabledSources;

    const disabledRequested = selectedSources.filter(
      (p) => sourcesConfig[p]?.enabled === false,
    );
    if (disabledRequested.length > 0) {
      return json(
        { error: `Skill source disabled: ${disabledRequested.join(", ")}` },
        { status: 403 },
      );
    }

    if (selectedSources.length === 0) {
      return json({ skills: [], total: 0, page: 1, totalPages: 0, sources: [] });
    }
    ...
    const filtered = curatedSkills.filter((skill) => {
      const haystack =
        `${skill.id} ${skill.name} ${skill.description} ${skill.category || ""}`.toLowerCase();
      ...
      if (normalizedTags.length > 0) {
        ...
        if (!normalizedTags.every((t) => tags.includes(t) || category.includes(t)))
          return false;
      }
      if (categoryParam) {
    ...
    console.error("[Hub Skills Search] Search failed:", error);
