<script lang="ts">
  ...
  let savedAiProviders = $state<Record<string, { apiKey: string; baseUrl: string }>>({});
  ...
  async function fetchSettings() {
    ...
    try {
      ...
      if (data.aiProviders) {
        ...
        for (const [key, value] of Object.entries(data.aiProviders) as [string, any][]) {
          providers[key] = {
            apiKey: value.apiKey || "",
          ...
          saved[key] = { apiKey: value.apiKey || "", baseUrl: value.baseUrl || "" };
        }
  ...
  async function saveSettings() {
    ...
    try {
      ...
      const aiProvidersPayload: Record<string, { apiKey?: string; baseUrl?: string }> = {};
      for (const [key, state] of Object.entries(aiProviders)) {
        const saved = savedAiProviders[key];
      ...
      timezone = editTimezone || null;
      for (const [key, state] of Object.entries(aiProviders)) {
        savedAiProviders[key] = { apiKey: state.apiKey, baseUrl: state.baseUrl };
          ...
          <div class="space-y-3">
            {#each Object.entries(aiProviders) as [key, provider] (key)}
              <div class="rounded-xl border border-border bg-card overflow-hidden">
                ...
                {#if expandedProviders.has(key)}
                  <div
                    class="border-t border-border px-4 py-4 space-y-4"
                  >
                    ...
                    <div>
                      ...
                      <div class="flex gap-2">
                        <input
                          ...
                          onfocus={(e) => {
                            if ((e.target as HTMLInputElement).value === MASKED) {
                              aiProviders[key] = { ...aiProviders[key], apiKey: "" };
                            }
                          }}
                    ...
                    <div>
                      ...
                      <p class="mt-1 text-[11px] text-muted-foreground/60">
                        Leave empty to use the default endpoint: {provider.meta.defaultBaseUrl}
                      </p>
                    </div>
                  </div>
                {/if}
        ...
        <section>
          ...
          <div class="rounded-xl border border-border bg-card p-4">
            ...
            <p class="mt-2 text-[11px] text-muted-foreground/60">
              ...
              > format. The model must be accessible via your configured API
              keys above.
            </p>
          </div>
        </section>
