...
| `POST` | `/api/nodes/:id/config-push` | Push config to a node (sends `config:push` WebSocket message) |
...
| `config:ack` | `{ configVersion, validations }` | Response to config:push with validation results |
