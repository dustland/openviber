...
<div class="space-y-8 p-6">
  <div>
    ...
    <p class="text-muted-foreground text-sm mt-1">
      Connect external accounts and channels for your vibers. Secrets are
      encrypted at rest.
    </p>
  </div>
  ...
  {:else}
    <section class="space-y-4">
      <div class="flex items-center gap-2">
        ...
        <h3 class="font-medium">OAuth Accounts</h3>
      </div>
      ...
      <div class="space-y-4">
          ...
          <div class="rounded-lg border bg-card p-5 space-y-4">
              ...
              >
                <div class="text-sm">
                  ...
                  <span class="ml-1 font-mono text-xs"
                    >{provider.connection.accountEmail || "Unknown"}</span
                  >
                </div>
                <button
            ...
            {:else}
              <p class="text-sm text-muted-foreground">
                The server administrator needs to configure {provider.name} OAuth
                credentials (<code class="text-xs">GOOGLE_CLIENT_ID</code> and
                <code class="text-xs">GOOGLE_CLIENT_SECRET</code>).
              </p>
            {/if}
          </div>
        {/each}
        ...
        {#if providers.length === 0}
          <div class="rounded-lg border border-dashed p-8 text-center">
            <p class="text-muted-foreground text-sm">
              No OAuth integrations available. Configure OAuth providers in
