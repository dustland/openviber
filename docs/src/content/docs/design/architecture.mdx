...
Channels connect external command centers (chat apps, web UIs, APIs) to the Viber runtime. They handle inbound messages and stream agent responses back to the source.

**Built-in Channels:**
- **DingTalk** - Enterprise bot via webhook (`/channels/dingtalk/webhook`)
- **WeCom** - WeChat Work integration (`/channels/wecom/webhook`)
- **Web** - HTTP + SSE for browser UIs (`/channels/web/message`, `/channels/web/stream/:id`)
...
Configure channels in `config/channels.yaml`:
```yaml
channels:
  dingtalk:
    enabled: true
...
### Agentic Composite Procedures (Playbooks)

Complex workflows are defined as "Playbooks" in Markdown. Unlike rigid scripts, these are read by a "Supervisor Agent" which plans execution dynamically.
